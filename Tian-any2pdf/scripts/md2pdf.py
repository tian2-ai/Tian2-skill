...
class PDFBuilder:
    ...
    def parse_md(self, md):
        ...
        while i < len(lines):
            ...
            # H4 = Sub-section (rendered like a smaller H3)
            if stripped.startswith('#### '):
                story.append(Spacer(1, 2*mm))
                story.append(Paragraph('<b>' + md_inline(stripped[5:].strip(), ah) + '</b>', ST['h3']))
                story.append(Spacer(1, 0.5*mm))
                i += 1; continue

            # H5 = Sub-sub-section (rendered as bold body)
            if stripped.startswith('##### '):
                story.append(Spacer(1, 1.5*mm))
                story.append(Paragraph('<b>' + md_inline(stripped[6:].strip(), ah) + '</b>', ST['body']))
                i += 1; continue

            # H6 fallback (rendered as bold body)
            if stripped.startswith('###### '):
                story.append(Paragraph('<b>' + md_inline(stripped[7:].strip(), ah) + '</b>', ST['body']))
                i += 1; continue
